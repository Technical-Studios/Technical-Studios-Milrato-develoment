    ...
    try {
      ...
      async function first_layer() {
        ...
        let MenuEmbed = new Discord.MessageEmbed()
          .setColor(es.color)
          .setAuthor(client.getAuthor('Menu Ticket Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/envelope_2709-fe0f.png', 'https://discord.gg/team-arcades-935157109761388554'))
          .setDescription(eval(client.la[ls]["cmds"]["setup"]["setup-ticket"]["variable2"]))

        //send the menu msg
        let menumsg = await message.reply({
          embeds: [MenuEmbed],
          components: [row1, row2, row3, row4, new MessageActionRow().addComponents(new MessageButton().setStyle("LINK").setURL("https://youtube.com/watch?v=1vp1ny4GJFg&t=441s").setLabel("Tutorial Video").setEmoji("840260133686870036"))]
        })
      ...
      async function second_layer(SetupNumber) {
        ...
        let MenuEmbed = new Discord.MessageEmbed()
          .setColor(es.color)
          //.setAuthor('Menu Ticket Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/envelope_2709-fe0f.png', 'https://discord.gg/team-arcades-935157109761388554')
          .setAuthor({ name: "Menu Ticket Setup", iconURL: "https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/envelope_2709-fe0f.png", url: "https://discord.gg/team-arcades-935157109761388554" })
          .setDescription(eval(client.la[ls]["cmds"]["setup"]["setup-ticket"]["variable2"]))

        //send the menu msg
        let menumsg = await message.reply({
          embeds: [MenuEmbed],
          components: [new MessageActionRow().addComponents(Selection), new MessageActionRow().addComponents(new MessageButton().setStyle("LINK").setURL("https://youtube.com/watch?v=1vp1ny4GJFg&t=441s").setLabel("Tutorial Video").setEmoji("840260133686870036"))]
        })
        //Create the collector
        const collector = menumsg.createMessageComponentCollector({
          filter: i => i?.isSelectMenu() && i?.message.author.id == client.user.id && i?.user,
        ...
        switch (optionhandletype) {
          case "Ticket Claim System": {
            ...
            */
            ...
            async function third_layer(SetupNumber) {
              ...
              let MenuEmbed = new Discord.MessageEmbed()
                .setColor(es.color)
                .setAuthor(SetupNumber + " Ticket Setup", "https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/twitter/282/incoming-envelope_1f4e8.png", "https://discord.gg/team-arcades-935157109761388554")
          ...
          case "Send the Config	Message": {
            ...
            if (!data || data.length < 1) {
              return message.reply("📤 **You need to add at least 1 Open-Ticket-Option**")
            }
          ...
          case "Edit Ticket Option": {
            ...
            let data = theDB.get(message.guild.id, pre + ".data");
            if (!data || data.length < 1) {
              return message.reply("📤 **There are no Open-Ticket-Options to remove**")
            }
            let embed = new MessageEmbed()
              .setColor(es.color)
              .setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
                ...
                async function handle_the_picks3(optionhandletype) {
                  ...
                  switch (optionhandletype) {
                    ...
                    case `Change Open Category`: {
                      ...
                      return message.reply("📤 **Invalid Category-ID added**")
                    } break;
            ...
            collector.on('end', collected => {
              menumsg.edit({
                embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
                components: [],
                content: `✅ **Selected: \`${collected.size > 0 ? collected.first().values[0] : "NOTHING"}\`**`
              })
            });
          }
            break;
