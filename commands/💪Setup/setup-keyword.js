  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      async function first_layer(){
        ...
        .setColor(es.color)
        .setAuthor('Keyword Setup', 'https://images-ext-1.discordapp.net/external/HF-XNy3iUP4D95zv2fuTUy1csYWuNa5IZj2HSCSkvhs/https/emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/google/298/flexed-biceps_1f4aa.png', 'https://discord.gg/team-arcades-935157109761388554')
      ...
      async function handle_the_picks(optionhandletype, menuoptiondata) {
                  ...
                  if (msg) {
                    ...
                    await tempmsg.channel.awaitMessages({filter: m => m.author.id === message.author.id,
                      ...
                      .then(async collected => {
                        ...
                        if (channel) {
                          for (const ch of collected.first().mentions.channels.map(this_Code_is_by_luismisaki_6966 => this_Code_is_by_luismisaki_6966)) {
        ...
        if (timeouterror)
          ...
          case "Delete Keyword": {
            ...
            let MenuEmbed = new Discord.MessageEmbed()
            .setColor(es.color)
            .setAuthor('Custom Command Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/google/298/flexed-biceps_1f4aa.png', 'https://discord.gg/team-arcades-935157109761388554')
            ...
            //Once the Collections ended edit the menu message
            collector.on('end', collected => {
              menumsg.edit({embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().values ? `✅ **Selected: \`${collected.first().values.length} Commands\`**` : "❌ **NOTHING SELECTED - CANCELLED**" }`})
            });
          }break;
          case "Show Settings": {
            let cuc = client.keyword.get(message.guild.id, "commands");
            ...
            for(let i = 0; i < cuc.length; i++){
              try{
                ...
                if(i > 13){
                  sendembed2 = true;
                  embed2.addField(`📌 \`${cuc[i].name}\` | ${cuc[i].embed ? "✅ Embed" : "❌ Embed"}`, ">>> "+ string)
                } else 
                embed.addField(`📌 \`${cuc[i].name}\` | ${cuc[i].embed ? "✅ Embed" : "❌ Embed"}`, ">>> "+ string)
              }catch (e){
                console.log(e.stack ? String(e.stack).grey : String(e).grey)
              }
            }
