  ...
  run: async (client, message, args, cmduser, text, prefix, player) => {
    ...
    try {
      ...
      .setAuthor(client.getAuthor(client.la[ls].cmds.info.serverinfo.author + " " +  message.guild.name, message.guild.iconURL({
        dynamic: true
      }), "https://discord.com/api/oauth2/authorize?client_id=960043017954283591&permissions=8&scope=bot%20applications.commands"))
      ...
      embed.addField("**📌 Rules Channel:**", `${message.guild.rulesChannel ? `<#${message.guild.rulesChannelId}>`: "📤 \`No Channel\`"}`, true)
      embed.addField("**📌 Public Updates Channel:**", `${message.guild.publicUpdatesChannel ? `<#${message.guild.publicUpdatesChannelId}>`: "📤 \`No Channel\`"}`, true)
      embed.addField("**📌 AFK Channel:**", `${message.guild.afkChannel ? `<#${message.guild.afkChannelId}>`: "📤 \`No Channel\`"}`, true)

      embed.addField("**📌 NSFW Level:**", `\`${message.guild.nsfwLevel}\``, true)
      embed.addField("**📌 Verifcation Level:**", `\`${message.guild.verificationLevel}\``, true)
      embed.addField("**📌 Explicit Content Filter:**", `\`${message.guild.explicitContentFilter}\``, true)
      ...
      embed.addField(client.la[ls].cmds.info.serverinfo.field12, "💜 \`" + message.guild.premiumSubscriptionCount + "\`", true)
      ...
      if(boosts >= 14){
          embed.addField(`**📌 Vanity:**`, `${message.guild.vanityURLCode ? `https://discord.gg/${message.guild.vanityURLCode}` : "📤 No Vanity-Invite"}`)
      }
      ...
      if(message.guild.banner) {
        ...
        let embed2 = new Discord.MessageEmbed()
        .setTitle(`**📌 SERVER DISCOVERY SPLASH:**`)
