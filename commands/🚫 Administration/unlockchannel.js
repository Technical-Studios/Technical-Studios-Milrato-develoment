    ...
    try {
      ...
      if(channel.isThread())
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`📤 **This Channel is a Thread u can't unlock it!**`)
        ]});
        ...
        if((users && users.length > 0) || (roles && roles.length > 0)){
          ...
          message.reply({embeds :[new MessageEmbed()
            .setColor(es.color)
            .setFooter(client.getFooter(es))
            .setTitle(`✅ **Successfully unlocked \`${channel.name}\` for ${users ? users.length : 0} Users and ${roles ? roles.length : 0} Roles**`)
          ]});
        } else {
          ...
          message.reply({embeds :[new MessageEmbed()
            .setColor(es.color)
            .setFooter(client.getFooter(es))
            .setTitle(`✅ **Successfully unlocked \`${channel.name}\`**`)
          ]});
        }
